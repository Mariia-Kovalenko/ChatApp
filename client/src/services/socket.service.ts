...
export function onTyping(
    cb: (data: { fromUserId: string; isTyping: boolean }) => void,
) {
...
// FIX: receive messages history
export const onReceiveHistory = (cb: (history: Message[]) => void) => {
    socket?.on("load_history", cb);
};
